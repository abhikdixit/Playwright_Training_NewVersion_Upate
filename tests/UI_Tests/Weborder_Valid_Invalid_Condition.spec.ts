...
test('Verify that User is unable to login with In-Valid Credentials', async ({ page }) => {
  await page.goto('http://secure.smartbearsoftware.com/samples/TestComplete11/WebOrders/Login.aspx');
  ...
  //Verify the Text
  await expect(page.locator('#ctl00_MainContent_status')).toContainText('Invalid Login or Password.');
  //await expect(page.locator('#ctl00_menu > li:nth-child(1) > a')).toHaveText('View all orders');
});
