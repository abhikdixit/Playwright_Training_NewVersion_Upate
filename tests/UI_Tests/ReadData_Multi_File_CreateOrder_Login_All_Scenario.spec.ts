...
const loginData = JSON.parse(readFileSync('./tests/TestData/WebOrder_Login_All_TCs.json', 'utf-8')) as LoginData[];
const orders = JSON.parse(readFileSync('./tests/TestData/create_order_All_Scenario.json', 'utf-8')) as OrderData[];

let validUsers: LoginData[] = []; // Store successful login credentials

test.describe('Step 1: Execute All Login Scenarios and Collect Valid Logins', () => {
  loginData.forEach((record: LoginData, idx: number) => {
...
test.describe('Step 2: Create Orders for Each Valid Login', () => {
  ...
  test(`Run Orders for All Valid Users`, async ({ browser }) => {
    ...
    for (let i = 0; i < validUsers.length; i++) {
      const user: LoginData = validUsers[i];
