...
let objects = readFileSync(`./tests/ZeroBank_Test_PageObject/TestData/PurchaseForeignCurrencyCash_Scenarios.json`, 'utf8');
const users: any[] = JSON.parse(objects);
...
test.describe('Transfer Funds and Make Payment', () => {
  ...
  for (const record of users) {
    test(`Purchase Foreign Currency Cash - ${record.selectCurrency}: ${record.TestCaseID}`, async () => {
